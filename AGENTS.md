...
## 基本方針
- 特に指示がない限り、日本語で回答する。
- 調査・報告の指示がある場合は、修正作業に進まず報告だけを行う
- コミットメッセージも日本語で作成する  
...
- 作業を始める前に、`serena` のメモリと実際のコードからプロジェクトの現状を読み出して、
  - いま何をやっているか  
  - 何が終わっていて、何が残っているか  
  をちゃんと把握すること。
...
4. **修正**
   - テストで落ちたところを直す。
   - 必ず `context7` でドキュメントを再チェックしたり、`serena` でコードのつながりを確認すること。
