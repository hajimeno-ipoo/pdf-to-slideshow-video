...
const drawSlideFrame = async (ctx, slideAsset, width, height, effectType, kenBurns, progress, slide, settings, currentTime, overlayAssets) => {
    ...
    const imgRatio = sw / sh; let finalW = availableW; let finalH = availableW / imgRatio;
    if (finalH > availableH) { finalH = availableH; finalW = availableH * imgRatio; }
    const centerX = width / 2; const centerY = height / 2; const drawW = finalW * kbScale; const drawH = finalH * kbScale;
    ...
    if (slide.overlays) {
        ...
        // Pass overlayAssets
        await drawOverlays(ctx, slide.overlays, finalW, finalH, currentTime, slide.duration, overlayAssets, transDuration);
        ctx.restore();
    }
};
