...
![スライドカード：画像を保存ボタン（ホバーで表示）](Doc/manual_images/16_slide_card_download_hover.png)

### 表示の切り替え（グリッド / カバーフロー）
- スライド一覧の右上にある **小さいスイッチ** で切り替えるよ
  - **グリッド**：たくさん並べて見る
  - **カバーフロー**：真ん中のカードを大きめに見せる
- カバーフローの移動はこれだよ
  - トラックパッド：左右スワイプ（横スクロール）
  - 画面の左右の「＜」「＞」ボタン
- スライドの **選択はクリック/タップだけ** だよ（スワイプだけで勝手に選ばれないよ）
- カバーフローでも、カードの中の操作（⤓/複製/削除/秒数/切り替え効果）はグリッドと同じだよ

![スライド一覧（カバーフロー）](Doc/manual_images/15_slide_list_coverflow.png)
...
![自動保存の復元（続きから始める）](Doc/manual_images/14_restore_modal.png)
